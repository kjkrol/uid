...
func TestUID64_GenerationIncrement(t *testing.T) {
	...
	oldUID := newUID(oldGen, index)
	nextUID := newUID(newGen, index)

	if oldUID.Index() != nextUID.Index() {
		t.Errorf("UIDs should share the same index: %d == %d", oldUID.Index(), nextUID.Index())
	}
	...
	if nextUID.Generation() <= oldUID.Generation() {
		t.Errorf("new UID should have higher generation: %d > %d", nextUID.Generation(), oldUID.Generation())
	}
}
